...
// Define following to publish additional sensor information; comment out to not
// publish
...
Roboteq::Roboteq() : Node("roboteq_ros2_driver") {
  ...
  param_update_timer = this->create_wall_timer(
      1000ms, std::bind(&Roboteq::update_parameters, this));
}
...
void Roboteq::connect() {
  RCLCPP_INFO_STREAM(this->get_logger(), "Opening serial port on "
                                             << port << " at " << baud
                                             << "...");
  try {
    controller.open();
...
void Roboteq::odom_setup() {
  ...
  //  RCLCPP_INFO(this->get_logger(), "Broadcasting odom tf"); // might use
  //  this-> instead of node
  odom_baselink_transform_ =
      std::make_unique<tf2_ros::TransformBroadcaster>(*this);
  ...
  // odom_pub = this->create_publisher<nav_msgs::msg::Odometry>(odom_topic,
  // 1000)
...
void Roboteq::odom_publish() {
  ...
  float linear =
      ((float)odom_encoder_right * wheel_circumference / (60 * gear_ratio) +
       (float)odom_encoder_left * wheel_circumference / (60 * gear_ratio)) /
      2;
